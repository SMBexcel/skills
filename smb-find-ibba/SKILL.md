...
`run` does both stages:
1. **enumerate** — one call to IBBA's public `/wp-json/brokers/geo` endpoint (radius covers all of North America) → writes `./ibba-export/ibba_brokers.csv` with every broker's name, company, city/state/zip, profile URL, and CBI/M&AMI credential flags (~2,800 rows).
2. **emails** — fetches each profile page (6 concurrent, gentle jittered delay, retries) and extracts email + phone + website from the page's hidden contact-form fields → writes `./ibba-export/ibba_contacts.csv`.

A full run takes roughly **5–10 minutes**. For a quick test first, add `--limit 10`.

**Resumable:** the `emails` stage skips any profile URL already in `ibba_contacts.csv`, and re-running automatically retries any rows that errored. Safe to stop and restart. If you want only the email stage later: `... ibba_scraper.py emails --out ./ibba-export`.
...
1. The path to `./ibba-export/ibba_contacts.csv` and the total broker count.
...
- **1.0** — Initial release. One public geo-endpoint call enumerates the full directory; each public profile page parsed in memory for email/phone/website via hidden contact-form fields. Stdlib-only Python (no deps), resumable email stage with auto-retry, date-free CSV output to `./ibba-export/`, gentle concurrency defaults, one-time legal-posture briefing (CAN-SPAM/CASL), claude.ai environment guard.
